...
let weatherCurrentDescription = document.getElementById(
...
const weatherGetCurrentsFunc = (lat = 30.38258, lon = -97.710243) => {
  fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=imperial&appid=89487f45423ccbfbdd6e1ea526f5177f`
  )
    ...
    .then(data => {
      ...
      //  handles regular hourly time
        ...
        weatherHourOne.innerHTML =
          pm === true ? `${currentHour + 1}pm` : `${currentHour + 1}am`;
        weatherHourTwo.innerHTML =
          pm === true ? `${currentHour + 2}pm` : `${currentHour + 2}am`;
        weatherHourThree.innerHTML =
          pm === true ? `${currentHour + 3}pm` : `${currentHour + 3}am`;
      }
    ...
    .catch(errror => {
      weatherDisplay404();
      console.error(
        "the 1call api to via openweathermap failed. check coords or api key"
      );
    });
};
...
const nameMatchFunc = userInput => {
  ...
  if (userInput.split(" ").length === 1) {
    cityString = userInput
      .split("")
      .map((e, i, a) => (i === 0 ? e.toUpperCase() : e))
      .join("");
  } else {
    cityString = userInput
      .split("")
      .map((e, i, a) =>
        i === 0 ? e.toUpperCase() : a[i - 1] === " " ? e.toUpperCase() : e
      )
      .join("");
  }
...
const getCitySearchCurrentsFunc = () => {
  ...
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?id=${nameMatchFunc(
      status
    )}&units=imperial&appid=89487f45423ccbfbdd6e1ea526f5177f`
  )
    .then(response => response.json())
    .then(data => {
    ...
    .catch(error => {
      console.error(
        `no match for "${weatherTextBox.value}." could not find the city id for gitCitySearchCurrentsFunc`
      );
...
clockAlarmSetButton.addEventListener("click", () => {
  ...
  setTheAlarm = setInterval(() => {
    ...
    if (
      ...
      a[0] === "0" &&
      clockDigit1.innerHTML === "" &&
      a[1] === clockDigit2.innerHTML &&
      a[3] === clockDigit3.innerHTML &&
      a[4] === clockDigit4.innerHTML &&
      a[a.length - 2] === clockAP.innerHTML
    ) {
      alarm = true;
    } else if (
      count === 0 &&
      a[0] === clockDigit1.innerHTML &&
      a[1] === clockDigit2.innerHTML &&
      a[3] === clockDigit3.innerHTML &&
      a[4] === clockDigit4.innerHTML &&
      a[a.length - 2] === clockAP.innerHTML
...
function countDownFunc(
  hour = +timerDownHour.value,
  min = +timerDownMin.value,
  sec = +timerDownSec.value
) {
  if (timerDownActive) {
    ...
    startDownTimer = setInterval(() => {
      ...
      let a = [hour, min, sec].map(e =>
        ("" + e).length === 0 ? `00` : ("" + e).length === 1 ? `0${e}` : e
      );
